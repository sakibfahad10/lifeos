...
async function me(req, res) {
    ...
    const existing = await prisma_js_1.prisma.user.findUnique({ where: { id }, select: { id: true, name: true, email: true, settings: true } });
    if (existing) {
        return (0, api_response_js_1.sendData)(res, existing);
    }
    const user = await prisma_js_1.prisma.user.create({ data: { id, email: userEmail || `${id}@lifeos.local`, name: userName }, select: { id: true, name: true, email: true, settings: true } });
