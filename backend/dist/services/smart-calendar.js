...
exports.schedulableTypes = void 0;
...
function parseNaturalLanguage(input, now = new Date()) {
    const text = input.trim();
    const lower = text.toLowerCase();
    const priority = /\b(high|urgent|important)\b/.test(lower) ? 'HIGH' : /\b(low)\b/.test(lower) ? 'LOW' : 'MEDIUM';
    ...
    if (weekday)
        startAt = nextWeekday(now, weekday[1]);
    if (!startAt && /\btoday\b/.test(lower))
        startAt = new Date(now);
    if (timeMatch) {
        let hour = Number(timeMatch[1]);
        if (/p/.test(timeMatch[3]) && hour !== 12)
            hour += 12;
        if (/a/.test(timeMatch[3]) && hour === 12)
            hour = 0;
        startAt = atTime(startAt || now, hour, Number(timeMatch[2] || 0));
    }
    if (startAt && startAt <= now && !/\btoday\b/.test(lower))
        startAt.setDate(startAt.getDate() + 1);
    ...
    const recurrenceRule = recurrence ? { frequency: 'WEEKLY', interval: 1, daysOfWeek: [weekdayNames[recurrence[1]], ...(recurrence[2] ? [weekdayNames[recurrence[2]]] : [])], endDate: untilMatch?.[1] } : /\bevery\s+2\s+weeks?\b/.test(lower) ? { frequency: 'WEEKLY', interval: 2, daysOfWeek: [], endDate: untilMatch?.[1] } : /\bmonthly\b/.test(lower) ? { frequency: 'MONTHLY', interval: 1, daysOfWeek: [], endDate: untilMatch?.[1] } : undefined;
    let title = text
...
function hasConflict(items, startAt, endAt, excludedId) {
    const start = startAt.getTime();
    const end = endAt.getTime();
    return items.filter(item => item.id !== excludedId && item.status !== client_1.CalendarItemStatus.CANCELLED).some(item => {
        const interval = intervalFor(item);
        return start < interval.end && end > interval.start;
    });
}
function findFreeSlots(items, minutes, count, availability = {}, now = new Date()) {
    ...
    if (startHour < 0 || endHour > 24 || startHour >= endHour)
        throw new Error('Availability hours must form a valid daily window.');
    const slots = [];
    for (let day = 0; day < horizonDays && slots.length < count; day++) {
        ...
        if (cursor < now)
            cursor = new Date(Math.ceil(now.getTime() / (30 * MINUTE)) * 30 * MINUTE);
        while (cursor.getTime() + minutes * MINUTE <= dayEnd.getTime() && slots.length < count) {
            const end = new Date(cursor.getTime() + minutes * MINUTE);
            if (!hasConflict(items, cursor, end)) {
                slots.push({ startAt: new Date(cursor), endAt: end });
                break;
            }
...
exports.schedulableTypes = [client_1.CalendarItemType.TASK, client_1.CalendarItemType.EVENT];
