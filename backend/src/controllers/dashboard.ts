  ...
  try {
    ...
    const [
      ...
      prisma.calendarItem.count({
        where: { userId, type: CalendarItemType.TASK },
      }),

      // 5. Completed tasks count
      prisma.calendarItem.count({
        where: { userId, type: CalendarItemType.TASK, status: CalendarItemStatus.COMPLETED },
      }),
      ...
      prisma.recurrenceRule.count({
        where: { calendarItem: { userId } },
      }),

      // 9. Recent AI import drafts
      prisma.aiImportDraft.findMany({
        where: { userId },
    ...
    // Synthesize real Recent Activity feed
    type ActivityItem = {
      id: string
    ...
    const completedToday = todayItems.filter(i => i.status === CalendarItemStatus.COMPLETED).length
