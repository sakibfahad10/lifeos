...
export default function ForgotPasswordPage() {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setPending(true); setMessage(''); const { error } = await createClient().auth.resetPasswordForEmail(email, { redirectTo: `${window.location.origin}/reset-password` }); setMessage(error ? 'Unable to send a reset email. Please try again.' : 'If an account exists, a password reset email is on its way.'); setPending(false) }
